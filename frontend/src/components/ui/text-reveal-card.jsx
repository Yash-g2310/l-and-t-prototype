...
export function TextRevealCard({ text, revealText, children, className }) {
  ...
  useEffect(() => {
    if (cardRef.current) {
      const { left: l, width: w } = cardRef.current.getBoundingClientRect();
      setLeft(l);
      setLocalWidth(w);
    }
  }, []);
...
export function TextRevealCardTitle({ children, className }) {
  return (
...
export function TextRevealCardDescription({ children, className }) {
  return (
